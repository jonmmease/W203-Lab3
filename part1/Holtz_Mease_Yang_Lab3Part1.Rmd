...
The coefficient shows decrease prbarr by 1% will increase the crmrte by ~0.74%.  ~47% of variation in crmrte are explained by low probabilty of prbarr, prbconv and higher polpc.  The model has 90 random observations, the parameters are unknown, there is no colilinearity between each of the parameters.  The residual vs. fitted value plot shows the residuals apprear to be centered at 0, also we don't see any patterns in the residuals.  We transformed crmrte, prbarr and polpc to meet the homoskedasticity assumption
...
The second model includes the following terms:
 ...
 6. Average wage of federal employees (`wfed`)
 7. Average wage of service industry employees (`wser`)
...
The average wage of federal employees was included as it was expected to add a degree of further accuracy to the model, but as most of the wage terms were colinear with density, most of the other wage terms were omitted purposely.

The average wage of the service industry was included because among the wage variables it was unique for being negatively correlated with crime rates. Policy makers could view this as evidence for the value of increasing the minimum wage in the hopes of reducing crime.


```{r}
(Model2 = lm(log(crmrte) ~ log(prbarr) + log(prbconv) + log(polpc) + log(pctmin80) + log(density) + wfed + wser, data = crime))
summary(Model2)
```

Additional 15% of variations in crmrte are influeced by high density and 5% by wage types of wfed and wser.  As Density and urban are problematic, due to multicollinearity, correlation = .82.  We excluded urban when examine density. One type goes up the other type tends to go up, they can tend to split the effect among themselves. There is considerable amount of multicollinearity among the different wages.  Also density and wages have some multicollinearity, which make sense since jobs in the city tend to have higher wages.  we included only wfed and wser because we believe they have the most influence to crmrte. All 6 CLM assumptions are evaluated.  Density is transformed to meet the homoskedasticity assumption.  The residual vs. fitted value plot shows the residuals apprear to be centered at 0.
...
```
Model2.1 was created to confirm if 'urban' was a better variable to use than 'density' as the two are colinear. Model2.1 is less accurate than Model2, so we will continue with Model2.

### Model 3

Model 3 was constructed using every variable with the exception of the "west", "central", and "urban" variables. The analysts believed that these variables are already correlated with density and they do not have any intrinsic value in informing policy. 
